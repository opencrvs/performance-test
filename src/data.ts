...
const FIRST_NAMES = [
  ...
  'Thomas', 'Karen', 'Charles', 'Lisa', 'Chanda', 'Mweene', 'Bwalya', 'Mutale',
  'Naledi', 'Thandiwe', 'Sipho', 'Nomsa', 'Thabo', 'Zanele',
];

const LAST_NAMES = [
  'Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis',
  'Mwansa', 'Phiri', 'Banda', 'Tembo', 'Mulenga', 'Chanda', 'Kabwe', 'Musonda',
  'Mutale', 'Zulu', 'Dlamini', 'Khumalo',
];

const ROLES = ['Registrar', 'Coach', 'Club President', 'Secretary', 'Treasurer'] as const;
...
export function generateDeclaration(): TennisClubDeclaration {
  ...
  const age = randInt(5, 90);
  const dob = randomDOB(age, age);
  // The senior-pass page is conditionally shown when applicant is 60+ years old
  const seniorPass = age >= 60 ? { 'senior-pass.id': randomId() } : {};
  ...
  return {
    ...
    'recommender.name': { firstname: recFirst, middlename: '', surname: recSurname },
    ...seniorPass,
