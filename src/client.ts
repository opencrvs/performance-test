...
export function searchByName(token: string, name: string): SearchResult {
  return search(token, { data: { 'applicant.name': { type: 'fuzzy', term: name } } });
}

/** Exact match on recommender.id (tennis-club proxy for NID). */
export function searchByNID(token: string, nid: string): SearchResult {
  return search(token, { data: { 'recommender.id': { type: 'exact', term: nid } } });
}

/** Exact DoB + fuzzy name (tennis-club proxy for DoB-and-name search). */
export function searchByDoBAndName(token: string, dob: string, name: string): SearchResult {
  return search(token, {
    data: {
...
export type EventStatus = 'CREATED' | 'NOTIFIED' | 'DECLARED' | 'REGISTERED' | 'ARCHIVED';

export function searchByStatus(token: string, status: EventStatus): SearchResult {
